...
export function mdToHtml(md) {
  ...
  html = html.replace(/^## (.+)$/gm, '<h2 id="$1">$1</h2>'.replace(/id="([^"]+)"/, (_, t) => {
    return null;
  }));
  ...
  html = html
    ...
    .map((block) => {
      const t = block.trim();
      if (!t) return "";
      if (t.startsWith("<h") || t.startsWith("<ul") || t.startsWith("<pre") || t.startsWith("<li")) return t;
      if (t.startsWith("<ul") || t.includes("<li>")) {
        if (!t.startsWith("<ul")) return `<ul>${t}</ul>`;
      }
      return `<p>${t.replace(/\n/g, " ")}</p>`;
    })
    .join("\n");
...
export function loadLayoutParts() {
  // reuse live CSS/JS; generate shell
  return null;
}
...
${body}
  ...
  <footer class="site-footer">
    <div class="wrap footer-grid">
      <div>
        <strong>${NAME}</strong>
        <p class="muted">Self-optimizing systems for search authority and compounding rankings.</p>
      </div>
      <div>
        <h3>Explore</h3>
